...
def mem_analyze(app, target_file, output_dir):
    ...
    if not os.path.exists(mem_result_file_path):
        ...
        writer = pd.ExcelWriter(mem_result_file_path, mode="a", engine="openpyxl")
        # 写入Excel表格
...
# 分析单独process进程cpu
def processs_cpu_analyze(app, target_file, output_dir):
    time_stamp = []
    cpu_usage_set = []
    ...
    for line in fileinput.input(target_file):
        if re.match(r"^\d{1,2}-\d{1,2} \d{1,2}:\d{1,2}:\d{1,2}.\d+", line):
            ...
            cpu_usage_set.append(line.split()[9])
            pass
        else:
            continue

    time_stamp += ("最大值",)
    cpu_usage_set.append(numpy.max(cpu_usage_set))
    time_stamp += ("最小值",)
    cpu_usage_set.append(numpy.min(cpu_usage_set))
    time_stamp += ("平均值",)
    cpu_usage_set.append(numpy.average(cpu_usage_set))
    time_stamp += ("中位数",)
    cpu_usage_set.append(numpy.median(cpu_usage_set))

    ps = pd.Series(cpu_usage_set, index=time_stamp)
    log.v(app + "cpu占用最大值:" + str(numpy.max(cpu_usage_set)))
    log.v(app + "cpu占用最小值:" + str(numpy.min(cpu_usage_set)))
    log.v(app + "cpu占用平均值:" + str(numpy.average(cpu_usage_set)))
    process_cpu_result_file_path = f"{output_dir}process_cpu_usage.xlsx"
    if not os.path.exists(process_cpu_result_file_path):
        ps.to_excel(process_cpu_result_file_path, sheet_name=app, index=True)
    else:
        log.v("mem_result.xlsx 文件已存在, 直接写入.")
        writer = pd.ExcelWriter(
            process_cpu_result_file_path, mode="a", engine="openpyxl"
        )
        # 写入Excel表格
        ps.to_excel(writer, sheet_name=app, index=True)
        # 保存Excel表格
...
def cpu_analyze(app, pid, target_file, output_dir):
    ...
    for line in fileinput.input(target_file):
        if re.match(r"^\d{1,2}-\d{1,2} \d{1,2}:\d{1,2}:\d{1,2}.\d+$", line):
            ...
            filter_process, _ = re.search(
                pattern=r"(.*)\s(\w+.\w+.\w+.*)$", string=line
            ).groups()
            _, t_name = re.search(
                pattern=r".*?(\d+:\d+.\d+)\s(.*)$", string=filter_process
            ).groups()
            key = f"{t_name}\n({line.split()[0]})"
            cpu = line.split()[8]
            if float(cpu) == 0:
    ...
    top_process = subprocess.run(
        f"adb shell top -H -k %CPU -s 11 -b -n 1 -p  {pid}",
        shell=True,
        capture_output=True,
    )
    for line in top_process.stdout.decode().splitlines():
        if re.match(r"^\s{0,3}\d+ .*", line):
            tid, time, name = re.search(
                r"^\s{0,3}(\d+)\s.*(\d+:\d+.\d+)\s(.*)$", line
            ).groups()
            tid_list.append(tid)
